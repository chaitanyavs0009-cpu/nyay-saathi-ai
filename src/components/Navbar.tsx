...
import { Globe, Menu, X, ArrowRight, UserCheck } from 'lucide-react';
...
export function Navbar({ language, onLanguageChange, onActionClick }: NavbarProps) {
          ...
          <nav className="hidden lg:flex items-center gap-0.5 xl:gap-1.5 shrink-1">
            {navLinks.map((item) => (
              <button
                ...
                className={`px-2.5 xl:px-3 py-1.5 rounded-lg text-xs xl:text-sm font-medium whitespace-nowrap transition-all ${
                  item.isHome
          ...
          <div className="flex items-center gap-2 sm:gap-3 shrink-0">
            ...
            <div className="flex items-center bg-slate-100/90 p-0.5 sm:p-1 rounded-lg border border-sky-100">
              <button
                ...
                className={`flex items-center gap-1 px-2 sm:px-2.5 py-1 text-xs font-semibold rounded-md whitespace-nowrap transition-all ${
                  language === 'en'
            ...
            <button
              ...
              onClick={() => onActionClick('login-signup', language === 'en' ? 'Citizen & Advocate Access' : 'नागरिक व अधिवक्ता लॉगिन')}
              className="hidden sm:inline-flex items-center gap-1.5 sm:gap-2 px-3.5 sm:px-4 py-2 text-xs xl:text-sm font-semibold rounded-lg bg-sky-600 text-white hover:bg-sky-700 transition-all shadow-sm shadow-sky-600/20 active:scale-95 whitespace-nowrap"
            >
            ...
            <button
              ...
              className="lg:hidden p-2 rounded-lg text-slate-700 hover:bg-sky-50 border border-sky-100 focus:outline-none"
        ...
        <div className="lg:hidden border-t border-sky-100 bg-white px-4 pt-3 pb-6 space-y-2 shadow-lg animate-in slide-in-from-top-2 duration-200">
          {navLinks.map((item) => (
            <button
              ...
              onClick={item.onClick}
              className={`w-full text-left px-3 py-2.5 rounded-lg text-sm sm:text-base font-medium flex items-center justify-between ${
                item.isHome
          ...
          <div className="pt-3 border-t border-sky-100 flex flex-col gap-2">
            <button
              ...
              className="w-full py-2.5 px-4 text-center rounded-lg bg-sky-600 hover:bg-sky-700 text-white font-semibold text-sm shadow-sm shadow-sky-600/20 flex items-center justify-center gap-2 transition-colors whitespace-nowrap"
            >
              <UserCheck className="w-4 h-4 text-sky-200" />
              <span>{language === 'en' ? 'Login / Signup' : 'लॉगिन / साइनअप'}</span>
            </button>
